...
class NN():
    ...
    def __init__(self,  hidden_layer, reg_lambda=0.0,
    ...
    def unpack_thetas(self, thetas, input_layer, hidden_layer, output_layer):
        ...
        t2_end = hidden_layer * (hidden_layer + 1)
        t1 = thetas[t1_start:t1_end].reshape((hidden_layer, input_layer + 1))
        t2 = thetas[t1_end:t2_end].reshape((hidden_layer, hidden_layer + 1))
        t3 = thetas[t2_end:].reshape((output_layer, hidden_layer + 1))
        return t1, t2, t3
    ...
    def feed_forward(self, X, t1, t2, t3):
        ...
        # output layer
        z4 = np.dot(t2, a2.T)
        a4 = self.activation_func(z3)
        return a1, z2, a2, z3, a3, z4, a4

    def cost_function(self, thetas, input_layer, hidden_layer, output_layer,
                      X, y, reg_lambda):
        t1, t2, t3 = self.unpack_thetas(thetas, input_layer, hidden_layer, output_layer)
        m = X.shape[0]
    ...
    def gradient(self, thetas, input_layer, hidden_layer, output_layer,
                 X, y, reg_lambda):
        t1, t2, t3 = self.unpack_thetas(thetas, input_layer, hidden_layer,
                                    output_layer)
        m = X.shape[0]
        t1_flat = t1[:, 1:]
        t2_flat = t2[:, 1:]
    ...
    def fit(self, X, y):
        ...
        theta2_0 = self.rand_init(self.hidden_layer, self.hidden_layer)
        theta3_0 = self.rand_init(self.hidden_layer, output_layer)
        ...
        self.t1, self.t2, self.t3 = self.unpack_thetas(_res.x, input_layer, self.hidden_layer, output_layer)
...
nn = NN(hidden_layer=50, maxiter=100, opti_method='CG', reg_lambda=2.21)
...
_,_,_,_, results = nn.feed_forward(X_test, nn.t1, nn.t2, nn.t3)
